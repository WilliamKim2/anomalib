...
- **China 셀(파트너 확장 시)**: 중화권 파트너사의 WeChat 데이터를 다루려면 PIPL 요구사항상 표준 APAC 셀에 통합하지 않고 별도의 China 리전 셀을 두거나, 현지 라이선스를 보유한 클라우드/파트너사를 경유하는 배치를 검토해야 함(3.1 아키텍처는 이 경우 4번째 셀로 확장)
...
| 중화권 파트너사 | WeChat | **China 전용 셀(신설 검토)** | PIPL(개인정보보호법)상 중국 내 데이터 로컬라이제이션 요구, 현지 라이선스 보유 클라우드(Tencent Cloud/Alibaba Cloud) 또는 현지 파트너사를 경유하는 별도 배치 필요 |
